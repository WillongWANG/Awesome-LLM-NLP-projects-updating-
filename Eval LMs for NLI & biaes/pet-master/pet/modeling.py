...
def train_pet(ensemble_model_config: WrapperConfig, ensemble_train_config: TrainConfig,
    ...
    # Step 3: Train the final sequence classifier model (model与step1 无关吧, knowledge distillation 在step2 具体不懂？)
    final_model_config.wrapper_type = SEQUENCE_CLASSIFIER_WRAPPER
    final_train_config.use_logits = True
...
def train_pet_ensemble(model_config: WrapperConfig, train_config: TrainConfig, eval_config: EvalConfig,
    ...
    """
    Train and evaluate an ensemble of PET models with/out knowledge distillation.
    ...
    for pattern_id in pattern_ids:
        for iteration in range(repetitions):
            ...
            if do_eval:
                ...
                with open(os.path.join(pattern_iter_output_dir, '(eval)results.json'), 'w') as fh:
    ...
    if do_eval:
        ...
        _write_results(os.path.join(output_dir, '(eval)result_test.txt'), results)
    else:
