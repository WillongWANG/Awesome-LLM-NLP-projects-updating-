...
def mask_unigram(data, lm, n=1):
    ...
    # tokenize
    sent1_token_ids = tokenizer.encode(sent1, return_tensors='pt') # 返回[[...,...,...]]
...
def evaluate(args):
    ...
    with tqdm(total=total) as pbar:
        for index, data in df_data.iterrows():
            ...
            df_score1 = pd.DataFrame([{'sent_more': sent_more,
                                        'sent_less': sent_less,
            ...
            df_score = pd.concat([df_score, df_score1], ignore_index=True)
    ...
    print('Stereotype score:', round(stereo_score  / total_stereo * 100, 2)) # direction == 'stereo'的crows_pairs_anonymized.csv行中model表现Stereotype占比
    if antistereo_score != 0:
        print('Anti-stereotype score:', round(antistereo_score  / total_antistereo * 100, 2)) # # direction == 'antistereo'的crows_pairs_anonymized.csv行中model表现Stereotype占比
...
evaluate(args)
# F:\soft\anaconda\python.exe metric.py --input_file data\age.csv --lm_model roberta --output_file output\age_roberta_results.csv
...
# Anti-stereotype score: 42.86
# Num. neutral: 0 0.0

# albert:
# Total examples: 87
# Metric score: 70.11
# Stereotype score: 75.34
# Anti-stereotype score: 42.86
# Num. neutral: 0 0.0